...
public class CreditCreditsEndpoint(IPlayerService playerService) : Endpoint<CreditCreditsRequest, bool>
{
    ...
    public override async Task HandleAsync(CreditCreditsRequest req, CancellationToken ct)
    {
        ...
        var success = await playerService.CreditCreditsAsync(req.PlayerId, req.Amount, req.Reason);
        if (!success)
        {
